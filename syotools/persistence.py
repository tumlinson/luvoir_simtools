...
from syotools.utils import JsonUnit, recover_quantities
...
class JSON(Protocol):
    ...
    def encode(self, entry):
        ...
        This makes use of JsonUnits to help with serialization of Quantities.
        
        Just in case, we'll also convert any actual Quantities into JsonUnits
        before serializing.
        """
        if isinstance(entry, JsonUnit):
            return entry.encode_json()
        if isinstance(entry, u.Quantity):
            return JsonUnit(entry).encode_json()
    ...
    def decode(self, attr, entry):
        ...
        Special cases:
          - If the entry is not a dict or a tuple, it is returned without
            modification.
          - If the entry is a JsonUnit serialization, convert back to a JsonUnit.
          - If the entry is a dict, recurse on each element.
          - If the entry is a list, convert it to a numpy array.
          - If the entry is some kind of attempt at a Quantity serialization,
            use JsonUnits.
        ...
        #Is this a JsonUnit serialization?
        try:
